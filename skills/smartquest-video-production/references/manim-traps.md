...
## 39. The GENERATED caption track was never told which theme it is in

Trap #31 is about a scene that re-themes and misses a captured default. This is the
sibling that skips the re-theming entirely: `src/captions.py` is generated by
`build_captions.py` and is the one file in a lesson that no human writes, so it never
picked up the `sq.use_light()` line every hand-written module starts with. The scenes
came out on a near-white field and the caption track came out in the DARK palette's
`CAPTION_INK` — `#F2F5FC` on `#FBFBFD`, about 1.03:1. The subtitles were burned into the
1080p60 master as pure white on white, and what survived was the antialiased edge of each
glyph, which reads as a faint embossed ghost rather than as missing text.

Three things hid it. The draft carries SOFT subtitles, so the player draws them in its own
colour and the draft looks perfect. The `.srt` sidecar is plain text, so every text-level
check passes. And Gate 5 measures duration, frame count, black frames and subtitle RATE —
none of which is contrast. Only a burned-in frame, looked at, shows it.

**Fix:** the plan already declares `"theme": "light"`; `build_captions.py` now reads it and
emits the `use_light()` call into the generated scene, and applies the same palette when it
measures the caption layout. **Any generator that emits a manim scene has to carry the
