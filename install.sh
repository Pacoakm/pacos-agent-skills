...
SKILL_SLUGS=("paco-video-production" "paco-interactive-educator")
DRY_RUN=0
SKILL_SELECTION="all"

usage() {
  printf '%s\n' "Usage: ./install.sh <codex|claude|hermes|all> [paco-video-production|paco-interactive-educator|all] [--dry-run]"
}
...
for argument in "$@"; do
  case "$argument" in
    --dry-run) DRY_RUN=1 ;;
    paco-video-production|paco-interactive-educator|all)
